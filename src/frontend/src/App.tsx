...
} from "./components/ConversionFeatures";
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
      ...
      {showBubble && (
        <button
          type="button"
          className="relative px-3 py-2 rounded-xl text-xs font-semibold max-w-[180px] text-right cursor-pointer"
          style={{
            background: "rgba(11,11,15,0.95)",
            border: "1px solid rgba(37,211,102,0.4)",
            color: "#fff",
            boxShadow: "0 4px 20px rgba(0,0,0,0.4)",
            animation: "chat-bob 2s ease-in-out infinite",
          }}
          onClick={() => setShowBubble(false)}
        >
          Hi! Need help deciding? Ask us! 👋{/* Arrow */}
          <div
            className="absolute -bottom-1.5 right-5 w-3 h-3 rotate-45"
            style={{
              background: "rgba(11,11,15,0.95)",
              borderRight: "1px solid rgba(37,211,102,0.4)",
              borderBottom: "1px solid rgba(37,211,102,0.4)",
            }}
          />
        </button>
      )}
      ...
      <div className="flex items-center gap-2">
        {/* CHAT NOW label */}
        <span
          className="hidden sm:inline-block text-xs font-black px-3 py-1.5 rounded-full"
          style={{
            background: "linear-gradient(135deg,#ff6a00,#ff9500)",
            color: "#fff",
            animation: "chat-bob 1.8s ease-in-out infinite",
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="w-14 h-14 rounded-full flex items-center justify-center whatsapp-pulse touch-target cursor-hover"
          style={{
            background: "#25d366",
